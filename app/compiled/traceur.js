define([], function() {
  ...
  (function(global) {
    ...
    function isSymbol(symbol) {
      return (typeof symbol === 'undefined' ? 'undefined' : $traceurRuntime.typeof(symbol)) === 'object' && symbol instanceof SymbolValue;
    }
    function typeOf(v) {
      if (isSymbol(v))
        return 'symbol';
      return (typeof v === 'undefined' ? 'undefined' : $traceurRuntime.typeof(v));
    }
    ...
    $defineProperty(Symbol.prototype, 'toString', method(function() {
      var symbolValue = this[$traceurRuntime.toProperty(symbolDataProperty)];
      if (!getOption('symbols'))
        return symbolValue[$traceurRuntime.toProperty(symbolInternalProperty)];
      if (!symbolValue)
        throw TypeError('Conversion from symbol to string');
      var desc = symbolValue[$traceurRuntime.toProperty(symbolDescriptionProperty)];
      if (desc === undefined)
        desc = '';
      return 'Symbol(' + desc + ')';
    }));
    ...
    function SymbolValue(description) {
      ...
      $freeze(this);
      $traceurRuntime.setProperty(symbolValues, key, this);
    }
    ...
    function toProperty(name) {
      if (isSymbol(name))
        return name[$traceurRuntime.toProperty(symbolInternalProperty)];
      return name;
    }
    function getOwnPropertyNames(object) {
      ...
      for (var i = 0; i < names.length; i++) {
        var name = names[$traceurRuntime.toProperty(i)];
        if (!symbolValues[$traceurRuntime.toProperty(name)])
          rv.push(name);
      }
      return rv;
    }
    ...
    function getOwnPropertySymbols(object) {
      ...
      var names = $getOwnPropertyNames(object);
      for (var i = 0; i < names.length; i++) {
        var symbol = symbolValues[$traceurRuntime.toProperty(names[$traceurRuntime.toProperty(i)])];
        if (symbol)
          rv.push(symbol);
      }
      return rv;
    ...
    function getOption(name) {
      return global.traceur && global.traceur.options[$traceurRuntime.toProperty(name)];
    }
    function setProperty(object, name, value) {
      ...
      if (isSymbol(name)) {
        sym = name;
        name = name[$traceurRuntime.toProperty(symbolInternalProperty)];
      }
      $traceurRuntime.setProperty(object, name, value);
    ...
    function polyfillObject(Object) {
      ...
      function assign(target, source) {
        ...
        var p,
            length = props.length;
        for (p = 0; p < length; p++) {
          $traceurRuntime.setProperty(target, props[$traceurRuntime.toProperty(p)], source[$traceurRuntime.toProperty(props[$traceurRuntime.toProperty(p)])]);
        }
        return target;
      }
      ...
      function mixin(target, source) {
        ...
        var p,
            ...
            length = props.length;
        for (p = 0; p < length; p++) {
          descriptor = $getOwnPropertyDescriptor(source, props[$traceurRuntime.toProperty(p)]);
          $defineProperty(target, props[$traceurRuntime.toProperty(p)], descriptor);
        }
        return target;
      }
    ...
    function exportStar(object) {
      for (var i = 1; i < arguments.length; i++) {
        var names = $getOwnPropertyNames(arguments[$traceurRuntime.toProperty(i)]);
        for (var j = 0; j < names.length; j++) {
          (function(mod, name) {
            $defineProperty(object, name, {
              get: function() {
                return mod[$traceurRuntime.toProperty(name)];
              },
              enumerable: true
            });
          })(arguments[$traceurRuntime.toProperty(i)], names[$traceurRuntime.toProperty(j)]);
        }
      }
      return object;
    }
    ...
    function spread() {
      ...
      for (var i = 0; i < arguments.length; i++) {
        var valueToSpread = toObject(arguments[$traceurRuntime.toProperty(i)]);
        for (var j = 0; j < valueToSpread.length; j++) {
          $traceurRuntime.setProperty(rv, k++, valueToSpread[$traceurRuntime.toProperty(j)]);
        }
      }
      return rv;
    }
    ...
    function getDescriptors(object) {
      ...
      for (var i = 0; i < names.length; i++) {
        var name = names[$traceurRuntime.toProperty(i)];
        $traceurRuntime.setProperty(descriptors, name, $getOwnPropertyDescriptor(object, name));
      }
      return descriptors;
    }
      ...
      pushTry: function(catchState, finallyState) {
        if (finallyState !== null) {
          ...
          for (var i = this.tryStack_.length - 1; i >= 0; i--) {
            if (this.tryStack_[$traceurRuntime.toProperty(i)].catch !== undefined) {
              finallyFallThrough = this.tryStack_[$traceurRuntime.toProperty(i)].catch;
              break;
            }
          }
      ...
      return function(ctx) {
        while (true) {
          try {
            ...
            var last = ctx.tryStack_[$traceurRuntime.toProperty(ctx.tryStack_.length - 1)];
            if (!last) {
              ctx.GState = ST_CLOSED;
              ctx.state = END_STATE;
              throw ex;
  ...
  (function() {
    ...
    function removeDotSegments(path) {
      ...
      for (var pos = 0; pos < segments.length; pos++) {
        var segment = segments[$traceurRuntime.toProperty(pos)];
        switch (segment) {
          case '':
          case '.':
            break;
    ...
    function joinAndCanonicalizePath(parts) {
      var path = parts[$traceurRuntime.toProperty(ComponentIndex.PATH)] || '';
      path = removeDotSegments(path);
      $traceurRuntime.setProperty(parts, ComponentIndex.PATH, path);
      return buildFromEncodedParts(parts[$traceurRuntime.toProperty(ComponentIndex.SCHEME)], parts[$traceurRuntime.toProperty(ComponentIndex.USER_INFO)], parts[$traceurRuntime.toProperty(ComponentIndex.DOMAIN)], parts[$traceurRuntime.toProperty(ComponentIndex.PORT)], parts[$traceurRuntime.toProperty(ComponentIndex.PATH)], parts[$traceurRuntime.toProperty(ComponentIndex.QUERY_DATA)], parts[$traceurRuntime.toProperty(ComponentIndex.FRAGMENT)]);
    }
    ...
    function resolveUrl(base, url) {
      ...
      if (parts[$traceurRuntime.toProperty(ComponentIndex.SCHEME)]) {
        return joinAndCanonicalizePath(parts);
      } else {
        $traceurRuntime.setProperty(parts, ComponentIndex.SCHEME, baseParts[$traceurRuntime.toProperty(ComponentIndex.SCHEME)]);
      }
      for (var i = ComponentIndex.SCHEME; i <= ComponentIndex.PORT; i++) {
        if (!parts[$traceurRuntime.toProperty(i)]) {
          $traceurRuntime.setProperty(parts, i, baseParts[$traceurRuntime.toProperty(i)]);
        }
      }
      if (parts[$traceurRuntime.toProperty(ComponentIndex.PATH)][0] == '/') {
        return joinAndCanonicalizePath(parts);
      }
      var path = baseParts[$traceurRuntime.toProperty(ComponentIndex.PATH)];
      var index = path.lastIndexOf('/');
      path = path.slice(0, index + 1) + parts[$traceurRuntime.toProperty(ComponentIndex.PATH)];
      $traceurRuntime.setProperty(parts, ComponentIndex.PATH, path);
      return joinAndCanonicalizePath(parts);
    }
  ...
  (function(global) {
    ...
    function getUncoatedModuleInstantiator(name) {
      ...
      return moduleInstantiators[$traceurRuntime.toProperty(url)];
    }
    ;
    ...
    function Module(uncoatedModule) {
      ...
      Object.getOwnPropertyNames(uncoatedModule).forEach((function(name) {
        ...
        if (!getter) {
          value = uncoatedModule[$traceurRuntime.toProperty(name)];
          getter = function() {
            return value;
          };
        }
    ...
    var ModuleStore = {
      ...
      get: function(normalizedName) {
        ...
        if (!m)
          return undefined;
        var moduleInstance = moduleInstances[$traceurRuntime.toProperty(m.url)];
        if (moduleInstance)
          return moduleInstance;
        moduleInstance = Module(m.getUncoatedModule(), liveModuleSentinel);
        return $traceurRuntime.setProperty(moduleInstances, m.url, moduleInstance);
      },
      set: function(normalizedName, module) {
        normalizedName = String(normalizedName);
        $traceurRuntime.setProperty(moduleInstantiators, normalizedName, new UncoatedModuleInstantiator(normalizedName, (function() {
          return module;
        })));
        $traceurRuntime.setProperty(moduleInstances, normalizedName, module);
      },
      get baseURL() {
        return baseURL;
      },
      ...
      registerModule: function(name, func) {
        ...
        if (moduleInstantiators[$traceurRuntime.toProperty(normalizedName)])
          throw new Error('duplicate module named ' + normalizedName);
        $traceurRuntime.setProperty(moduleInstantiators, normalizedName, new UncoatedModuleInstantiator(normalizedName, func));
      },
      ...
      register: function(name, deps, func) {
        if (!deps || !deps.length) {
          ...
          $traceurRuntime.setProperty(this.bundleStore, name, {
            deps: deps,
            execute: func
          });
        }
    ...
    ($traceurRuntime.createClass)(ArrayIterator, ($__4 = {}, Object.defineProperty($__4, "next", {
      value: function() {
        ...
        if (itemKind == ARRAY_ITERATOR_KIND_VALUES)
          return createIteratorResultObject(array[$traceurRuntime.toProperty(index)], false);
        if (itemKind == ARRAY_ITERATOR_KIND_ENTRIES)
          return createIteratorResultObject([index, array[$traceurRuntime.toProperty(index)]], false);
  ...
  System.register("traceur@0.0.32/node_modules/rsvp/lib/rsvp/asap", [], function() {
    ...
    function flush() {
      for (var i = 0; i < queue.length; i++) {
        var tuple = queue[$traceurRuntime.toProperty(i)];
  ...
  System.register("traceur@0.0.32/src/runtime/polyfills/Promise", [], function() {
    ...
    ($traceurRuntime.createClass)(Promise, {
      ...
      all: function(values) {
        ...
        try {
          for (var i = 0; i < values.length; i++) {
            ++count;
            this.cast(values[$traceurRuntime.toProperty(i)]).then(function(i, x) {
              $traceurRuntime.setProperty(resolutions, i, x);
              if (--count === 0)
      ...
      race: function(values) {
        ...
        try {
          for (var i = 0; i < values.length; i++) {
            this.cast(values[$traceurRuntime.toProperty(i)]).then((function(x) {
              deferred.resolve(x);
            }), (function(r) {
              deferred.reject(r);
            }));
    ...
    function promiseDone(promise, status, value, reactions) {
      ...
      for (var i = 0; i < reactions.length; i++) {
        promiseReact(reactions[$traceurRuntime.toProperty(i)][0], reactions[$traceurRuntime.toProperty(i)][1], value);
      }
    ...
    function promiseCoerce(constructor, x) {
      if (isPromise(x)) {
        ...
        var p = x[$traceurRuntime.toProperty(thenableSymbol)];
        if (p) {
          return p;
        } else {
          var deferred = getDeferred(constructor);
          $traceurRuntime.setProperty(x, thenableSymbol, deferred.promise);
          try {
  ...
  System.register("traceur@0.0.32/src/runtime/polyfills/String", [], function() {
    ...
    function raw(callsite) {
      ...
      var i = 0;
      while (true) {
        s += raw[$traceurRuntime.toProperty(i)];
        if (i + 1 === len)
          return s;
        s += arguments[$traceurRuntime.toProperty(++i)];
      }
    }
    function fromCodePoint() {
      ...
      while (++index < length) {
        var codePoint = Number(arguments[$traceurRuntime.toProperty(index)]);
  ...
  System.register("traceur@0.0.32/src/runtime/polyfills/polyfills", [], function() {
    ...
    function maybeDefineMethod(object, name, value) {
      if (!($traceurRuntime.toProperty(name) in object)) {
    ...
    function maybeAddFunctions(object, functions) {
      for (var i = 0; i < functions.length; i += 2) {
        var name = functions[$traceurRuntime.toProperty(i)];
        var value = functions[$traceurRuntime.toProperty(i + 1)];
  ...
  System.register("traceur@0.0.32/src/options", [], function() {
    ...
    var options = {
      set experimental(v) {
        ...
        Object.keys(experimentalOptions).forEach((function(name) {
          $traceurRuntime.setProperty(options, name, v);
        }));
      },
      get experimental() {
        var value;
        Object.keys(experimentalOptions).every((function(name) {
          var currentValue = options[$traceurRuntime.toProperty(name)];
          if (value === undefined) {
            value = currentValue;
            return true;
          }
    ...
    function reset() {
      ...
      Object.keys(options).forEach((function(name) {
        $traceurRuntime.setProperty(options, name, useDefault && defaultValues[$traceurRuntime.toProperty(name)]);
      }));
    }
    ...
    function setFromObject(object) {
      Object.keys(object).forEach((function(name) {
        $traceurRuntime.setProperty(options, name, object[$traceurRuntime.toProperty(name)]);
      }));
    }
    ...
    function setOption(name, value) {
      ...
      if ($traceurRuntime.toProperty(name) in options) {
        $traceurRuntime.setProperty(options, name, value);
      } else {
    ...
    function addOptions(flags) {
      Object.keys(options).forEach(function(name) {
        ...
        if (($traceurRuntime.toProperty(name) in parseOptions) && ($traceurRuntime.toProperty(name) in transformOptions)) {
          flags.option('--' + dashedName + ' [true|false|parse]', descriptions[$traceurRuntime.toProperty(name)]);
          flags.on(dashedName, (function(value) {
            return setOption(dashedName, value);
          }));
        } else if (options[$traceurRuntime.toProperty(name)] !== null) {
          flags.option('--' + dashedName, descriptions[$traceurRuntime.toProperty(name)]);
    ...
    function filterOption(dashedName) {
      ...
      return name === 'experimental' || !($traceurRuntime.toProperty(name) in options);
    }
    ...
    function addFeatureOption(name, kind) {
      if (kind === EXPERIMENTAL)
        $traceurRuntime.setProperty(experimentalOptions, name, true);
      Object.defineProperty(parseOptions, name, {
        get: function() {
          return !!options[$traceurRuntime.toProperty(name)];
        },
        enumerable: true,
        configurable: true
      });
      Object.defineProperty(transformOptions, name, {
        get: function() {
          var v = options[$traceurRuntime.toProperty(name)];
          if (v === 'parse')
            return false;
          return v;
        },
      ...
      var defaultValue = options[$traceurRuntime.toProperty(name)] || kind === ON_BY_DEFAULT;
      $traceurRuntime.setProperty(options, name, defaultValue);
      $traceurRuntime.setProperty(defaultValues, name, defaultValue);
    }
    function addBoolOption(name) {
      $traceurRuntime.setProperty(defaultValues, name, false);
      $traceurRuntime.setProperty(options, name, false);
    }
      ...
      visitList: function(list) {
        if (list) {
          for (var i = 0; i < list.length; i++) {
            this.visitAny(list[$traceurRuntime.toProperty(i)]);
          }
        }
      },
  ...
  System.register("traceur@0.0.32/src/util/JSON", [], function() {
    ...
    function transform_(v, replacer) {
      ...
      if (Array.isArray(v)) {
        ...
        rv = Array(len);
        for (var i = 0; i < len; i++) {
          tv = transform_(replacer(String(i), v[$traceurRuntime.toProperty(i)]), replacer);
          $traceurRuntime.setProperty(rv, i, tv === undefined ? null : tv);
        }
        return rv;
      }
      if (v instanceof Object) {
        ...
        Object.keys(v).forEach((function(k) {
          tv = transform_(replacer(k, v[$traceurRuntime.toProperty(k)]), replacer);
          if (tv !== undefined) {
            $traceurRuntime.setProperty(rv, k, tv);
          }
        }));
        return rv;
      }
      ...
      replacer: function(k, v) {
        if (v instanceof $ParseTree || v instanceof Token) {
          ...
          Object.keys(v).forEach(function(name) {
            if (name !== 'location')
              $traceurRuntime.setProperty(rv, name, v[$traceurRuntime.toProperty(name)]);
          });
          return rv;
        }
        return v;
  ...
  System.register("traceur@0.0.32/src/semantics/FreeVariableChecker", [], function() {
    ...
    ($traceurRuntime.createClass)(FreeVariableChecker, {
      ...
      visitIdentifierExpression: function(tree) {
        ...
        if (!($traceurRuntime.toProperty(name) in scope.references)) {
          $traceurRuntime.setProperty(scope.references, name, tree.location);
        }
      },
      visitUnaryExpression: function(tree) {
      ...
      declareVariable_: function(tree) {
        ...
        if (name) {
          var scope = this.scope_;
          if (!($traceurRuntime.toProperty(name) in scope.declarations)) {
            $traceurRuntime.setProperty(scope.declarations, name, tree.location);
          }
        }
      },
      validateScope_: function() {
        ...
        for (var name in scope.references) {
          if (!($traceurRuntime.toProperty(name) in scope.declarations)) {
            var location = scope.references[$traceurRuntime.toProperty(name)];
            if (!scope.parent) {
              if (!location) {
                throw new Error(("generated variable " + name + " is not defined"));
              }
              errors.push([location.start, (name + " is not defined")]);
            } else if (!($traceurRuntime.toProperty(name) in scope.parent.references)) {
              $traceurRuntime.setProperty(scope.parent.references, name, location);
            }
          }
        }
        if (errors.length) {
  ...
  System.register("traceur@0.0.32/src/syntax/Keywords", [], function() {
    ...
    keywords.forEach((function(value) {
      $traceurRuntime.setProperty(keywordsByName, value, NORMAL_KEYWORD);
    }));
    strictKeywords.forEach((function(value) {
      $traceurRuntime.setProperty(keywordsByName, value, STRICT_KEYWORD);
    }));
    function getKeywordType(value) {
      return keywordsByName[$traceurRuntime.toProperty(value)];
    }
  ...
  System.register("traceur@0.0.32/src/syntax/LiteralToken", [], function() {
    ...
    ($traceurRuntime.createClass)(StringParser, ($__28 = {}, Object.defineProperty($__28, Symbol.iterator, {
      ...
      value: function() {
        ...
        return {
          value: this.value[$traceurRuntime.toProperty(this.index)],
          done: false
        };
      },
      configurable: true,
      ...
      value: function() {
        ...
        var result = '';
        for (var $__29 = this[$traceurRuntime.toProperty(Symbol.iterator)](),
  ...
  System.register("traceur@0.0.32/src/syntax/Scanner", [], function() {
    ...
    for (var i = 0; i < 128; i++) {
      $traceurRuntime.setProperty(isWhitespaceArray, i, i >= 9 && i <= 13 || i === 0x20);
    }
    var isWhitespaceArray = [];
    for (var i = 0; i < 128; i++) {
      $traceurRuntime.setProperty(isWhitespaceArray, i, i >= 9 && i <= 13 || i === 0x20);
    ...
    function isWhitespace(code) {
      if (code < 128)
        return isWhitespaceArray[$traceurRuntime.toProperty(code)];
      switch (code) {
        case 0xA0:
        case 0xFEFF:
        case 0x2028:
    ...
    var isHexDigitArray = [];
    for (var i = 0; i < 128; i++) {
      $traceurRuntime.setProperty(isHexDigitArray, i, i >= 48 && i <= 57 || i >= 65 && i <= 70 || i >= 97 && i <= 102);
    }
    function isHexDigit(code) {
      return code < 128 && isHexDigitArray[$traceurRuntime.toProperty(code)];
    }
    ...
    for (var i = 0; i < 128; i++) {
      $traceurRuntime.setProperty(isIdentifierStartArray, i, i === 36 || i >= 65 && i <= 90 || i === 95 || i >= 97 && i <= 122);
    }
    function isIdentifierStart(code) {
      return code < 128 ? isIdentifierStartArray[$traceurRuntime.toProperty(code)] : inTable(idStartTable, code);
    }
    var isIdentifierPartArray = [];
    for (var i = 0; i < 128; i++) {
      $traceurRuntime.setProperty(isIdentifierPartArray, i, isIdentifierStart(i) || isDecimalDigit(i));
    }
    function isIdentifierPart(code) {
      return code < 128 ? isIdentifierPartArray[$traceurRuntime.toProperty(code)] : inTable(idStartTable, code) || inTable(idContinueTable, code) || code === 8204 || code === 8205;
    }
    function inTable(table, code) {
      for (var i = 0; i < table.length; ) {
        if (code < table[$traceurRuntime.toProperty(i++)])
          return false;
        if (code <= table[$traceurRuntime.toProperty(i++)])
          return true;
      }
    ...
    function scanIdentifierOrKeyword(beginIndex, code) {
      ...
      if (escapedCharCodes) {
        ...
        value = value.replace(/\\u..../g, function(s) {
          return String.fromCharCode(escapedCharCodes[$traceurRuntime.toProperty(i++)]);
        });
      }
    ...
    ($traceurRuntime.createClass)(ParseTreeWriter, {
      ...
      visitFormalParameterList: function(tree) {
        ...
        for (var i = 0; i < tree.parameters.length; i++) {
          var parameter = tree.parameters[$traceurRuntime.toProperty(i)];
          if (first) {
            first = false;
          } else {
            this.write_(COMMA);
      ...
      writeList_: function(list, delimiter, writeNewLine) {
        ...
        var first = true;
        for (var i = 0; i < list.length; i++) {
          var element = list[$traceurRuntime.toProperty(i)];
          if (first) {
            first = false;
          } else {
            if (delimiter !== null) {
  ...
  System.register("traceur@0.0.32/src/outputgeneration/SourceMapIntegration", [], function() {
    ...
    function makeDefine(mapping, id) {
      var require = function(id) {
        return mapping[$traceurRuntime.toProperty(id)];
      };
      var exports = $traceurRuntime.setProperty(mapping, id, {});
      var module = null;
    ...
    define(function(require, exports, module) {
      function getArg(aArgs, aName, aDefaultValue) {
        if ($traceurRuntime.toProperty(aName) in aArgs) {
          return aArgs[$traceurRuntime.toProperty(aName)];
    ...
    define(function(require, exports, module) {
      ...
      ArraySet.fromArray = function ArraySet_fromArray(aArray, aAllowDuplicates) {
        ...
        for (var i = 0,
          ...
          set.add(aArray[$traceurRuntime.toProperty(i)], aAllowDuplicates);
        }
        return set;
      };
      ArraySet.prototype.add = function ArraySet_add(aStr, aAllowDuplicates) {
        ...
        if (!isDuplicate) {
          $traceurRuntime.setProperty(this._set, util.toSetString(aStr), idx);
        }
      };
      ...
      ArraySet.prototype.indexOf = function ArraySet_indexOf(aStr) {
        if (this.has(aStr)) {
          return this._set[$traceurRuntime.toProperty(util.toSetString(aStr))];
        }
        throw new Error('"' + aStr + '" is not in the set.');
      };
      ArraySet.prototype.at = function ArraySet_at(aIdx) {
        if (aIdx >= 0 && aIdx < this._array.length) {
          return this._array[$traceurRuntime.toProperty(aIdx)];
        }
    ...
    define(function(require, exports, module) {
      ...
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.split('').forEach(function(ch, index) {
        $traceurRuntime.setProperty(charToIntMap, ch, index);
        $traceurRuntime.setProperty(intToCharMap, index, ch);
      });
      exports.encode = function base64_encode(aNumber) {
        if ($traceurRuntime.toProperty(aNumber) in intToCharMap) {
          return intToCharMap[$traceurRuntime.toProperty(aNumber)];
        }
        throw new TypeError("Must be between 0 and 63: " + aNumber);
      };
      exports.decode = function base64_decode(aChar) {
        if ($traceurRuntime.toProperty(aChar) in charToIntMap) {
          return charToIntMap[$traceurRuntime.toProperty(aChar)];
        }
    ...
    define(function(require, exports, module) {
      function recursiveSearch(aLow, aHigh, aNeedle, aHaystack, aCompare) {
        ...
        var cmp = aCompare(aNeedle, aHaystack[$traceurRuntime.toProperty(mid)], true);
        if (cmp === 0) {
          return aHaystack[$traceurRuntime.toProperty(mid)];
        } else if (cmp > 0) {
          if (aHigh - mid > 1) {
          ...
          return aLow < 0 ? null : aHaystack[$traceurRuntime.toProperty(aLow)];
        }
      }
    ...
    define(function(require, exports, module) {
      ...
      SourceMapGenerator.prototype.setSourceContent = function SourceMapGenerator_setSourceContent(aSourceFile, aSourceContent) {
        ...
        if (aSourceContent !== null) {
          ...
          $traceurRuntime.setProperty(this._sourcesContents, util.toSetString(source), aSourceContent);
        } else {
          delete this._sourcesContents[$traceurRuntime.toProperty(util.toSetString(source))];
      ...
      SourceMapGenerator.prototype._serializeMappings = function SourceMapGenerator_serializeMappings() {
        ...
        for (var i = 0,
          ...
          mapping = this._mappings[$traceurRuntime.toProperty(i)];
          if (mapping.generatedLine !== previousGeneratedLine) {
            ...
            if (i > 0) {
              if (!util.compareByGeneratedPositions(mapping, this._mappings[$traceurRuntime.toProperty(i - 1)])) {
                continue;
              }
              result += ',';
            }
      ...
      SourceMapGenerator.prototype._generateSourcesContent = function SourceMapGenerator_generateSourcesContent(aSources, aSourceRoot) {
        return aSources.map(function(source) {
          ...
          return Object.prototype.hasOwnProperty.call(this._sourcesContents, key) ? this._sourcesContents[$traceurRuntime.toProperty(key)] : null;
        }, this);
      };
    ...
    define(function(require, exports, module) {
      ...
      SourceMapConsumer.prototype._findMapping = function SourceMapConsumer_findMapping(aNeedle, aMappings, aLineName, aColumnName, aComparator) {
        if (aNeedle[$traceurRuntime.toProperty(aLineName)] <= 0) {
          throw new TypeError('Line must be greater than or equal to 1, got ' + aNeedle[$traceurRuntime.toProperty(aLineName)]);
        }
        if (aNeedle[$traceurRuntime.toProperty(aColumnName)] < 0) {
          throw new TypeError('Column must be greater than or equal to 0, got ' + aNeedle[$traceurRuntime.toProperty(aColumnName)]);
        }
      ...
      SourceMapConsumer.prototype.sourceContentFor = function SourceMapConsumer_sourceContentFor(aSource) {
        ...
        if (this._sources.has(aSource)) {
          return this.sourcesContent[$traceurRuntime.toProperty(this._sources.indexOf(aSource))];
        }
        var url;
        if (this.sourceRoot && (url = util.urlParse(this.sourceRoot))) {
          var fileUriAbsPath = aSource.replace(/^file:\/\//, "");
          if (url.scheme == "file" && this._sources.has(fileUriAbsPath)) {
            return this.sourcesContent[$traceurRuntime.toProperty(this._sources.indexOf(fileUriAbsPath))];
          }
    ...
    define(function(require, exports, module) {
        ...
        aSourceMapConsumer.eachMapping(function(mapping) {
          if (lastMapping === null) {
            ...
            if (lastGeneratedColumn < mapping.generatedColumn) {
              ...
              $traceurRuntime.setProperty(remainingLines, 0, nextLine.substr(mapping.generatedColumn));
            ...
            if (lastGeneratedLine < mapping.generatedLine) {
              ...
              $traceurRuntime.setProperty(remainingLines, 0, nextLine.substr(mapping.generatedColumn - lastGeneratedColumn));
              lastGeneratedColumn = mapping.generatedColumn;
              addMappingWithCode(lastMapping, code);
            }
          }
      ...
      SourceNode.prototype.prepend = function SourceNode_prepend(aChunk) {
        if (Array.isArray(aChunk)) {
          for (var i = aChunk.length - 1; i >= 0; i--) {
            this.prepend(aChunk[$traceurRuntime.toProperty(i)]);
          }
        } else if (aChunk instanceof SourceNode || typeof aChunk === "string") {
      ...
      SourceNode.prototype.walk = function SourceNode_walk(aFn) {
        ...
        for (var i = 0,
          ...
          chunk = this.children[$traceurRuntime.toProperty(i)];
      ...
      SourceNode.prototype.join = function SourceNode_join(aSep) {
        ...
        if (len > 0) {
          newChildren = [];
          for (i = 0; i < len - 1; i++) {
            newChildren.push(this.children[$traceurRuntime.toProperty(i)]);
      ...
      SourceNode.prototype.replaceRight = function SourceNode_replaceRight(aPattern, aReplacement) {
        var lastChild = this.children[$traceurRuntime.toProperty(this.children.length - 1)];
        if (lastChild instanceof SourceNode) {
          lastChild.replaceRight(aPattern, aReplacement);
        } else if (typeof lastChild === 'string') {
          $traceurRuntime.setProperty(this.children, this.children.length - 1, lastChild.replace(aPattern, aReplacement));
        } else {
      ...
      SourceNode.prototype.setSourceContent = function SourceNode_setSourceContent(aSourceFile, aSourceContent) {
        $traceurRuntime.setProperty(this.sourceContents, util.toSetString(aSourceFile), aSourceContent);
      };
      SourceNode.prototype.walkSourceContents = function SourceNode_walkSourceContents(aFn) {
        for (var i = 0,
            len = this.children.length; i < len; i++) {
          if (this.children[$traceurRuntime.toProperty(i)] instanceof SourceNode) {
            this.children[$traceurRuntime.toProperty(i)].walkSourceContents(aFn);
          }
        }
        var sources = Object.keys(this.sourceContents);
        for (var i = 0,
            len = sources.length; i < len; i++) {
          aFn(util.fromSetString(sources[$traceurRuntime.toProperty(i)]), this.sourceContents[$traceurRuntime.toProperty(sources[$traceurRuntime.toProperty(i)])]);
        }
      };
    ...
    var SourceMapGenerator = m[$traceurRuntime.toProperty('./source-map-generator')].SourceMapGenerator;
    var SourceMapConsumer = m[$traceurRuntime.toProperty('./source-map-consumer')].SourceMapConsumer;
    var SourceNode = m[$traceurRuntime.toProperty('./source-node')].SourceNode;
    return {
    ...
    ($traceurRuntime.createClass)(ParseTreeValidator, {
      ...
      visitArgumentList: function(tree) {
        for (var i = 0; i < tree.args.length; i++) {
          var argument = tree.args[$traceurRuntime.toProperty(i)];
          this.checkVisit_(argument.isAssignmentOrSpread(), argument, 'assignment or spread expected');
        }
      },
      visitArrayLiteralExpression: function(tree) {
        for (var i = 0; i < tree.elements.length; i++) {
          var element = tree.elements[$traceurRuntime.toProperty(i)];
      ...
      visitBlock: function(tree) {
        for (var i = 0; i < tree.statements.length; i++) {
          var statement = tree.statements[$traceurRuntime.toProperty(i)];
      ...
      visitCommaExpression: function(tree) {
        for (var i = 0; i < tree.expressions.length; i++) {
          var expression = tree.expressions[$traceurRuntime.toProperty(i)];
      ...
      visitExportSpecifierSet: function(tree) {
        ...
        for (var i = 0; i < tree.specifiers.length; i++) {
          var specifier = tree.specifiers[$traceurRuntime.toProperty(i)];
      ...
      visitObjectLiteralExpression: function(tree) {
        for (var i = 0; i < tree.propertyNameAndValues.length; i++) {
          var propertyNameAndValue = tree.propertyNameAndValues[$traceurRuntime.toProperty(i)];
      ...
      visitObjectPattern: function(tree) {
        for (var i = 0; i < tree.fields.length; i++) {
          var field = tree.fields[$traceurRuntime.toProperty(i)];
      ...
      visitScript: function(tree) {
        for (var i = 0; i < tree.scriptItemList.length; i++) {
          var scriptItemList = tree.scriptItemList[$traceurRuntime.toProperty(i)];
      ...
      visitSwitchStatement: function(tree) {
        ...
        for (var i = 0; i < tree.caseClauses.length; i++) {
          var caseClause = tree.caseClauses[$traceurRuntime.toProperty(i)];
  ...
  System.register("traceur@0.0.32/src/util/ObjectMap", [], function() {
    ...
    ($traceurRuntime.createClass)(ObjectMap, {
      set: function(key, value) {
        var uid = key.uid;
        $traceurRuntime.setProperty(this.keys_, uid, key);
        $traceurRuntime.setProperty(this.values_, uid, value);
      },
      get: function(key) {
        return this.values_[$traceurRuntime.toProperty(key.uid)];
      },
      has: function(key) {
        return $traceurRuntime.toProperty(key.uid) in this.keys_;
      },
      addAll: function(other) {
        for (var uid in other.keys_) {
          $traceurRuntime.setProperty(this.keys_, uid, other.keys_[$traceurRuntime.toProperty(uid)]);
          $traceurRuntime.setProperty(this.values_, uid, other.values_[$traceurRuntime.toProperty(uid)]);
        }
      },
      keys: function() {
        var $__43 = this;
        return Object.keys(this.keys_).map((function(uid) {
          return $__43.keys_[$traceurRuntime.toProperty(uid)];
        }));
      },
      values: function() {
        var $__43 = this;
        return Object.keys(this.values_).map((function(uid) {
          return $__43.values_[$traceurRuntime.toProperty(uid)];
        }));
      },
      remove: function(key) {
        var uid = key.uid;
        delete this.keys_[$traceurRuntime.toProperty(uid)];
        delete this.values_[$traceurRuntime.toProperty(uid)];
      }
    }, {});
  ...
  System.register("traceur@0.0.32/src/codegeneration/ParseTreeFactory", [], function() {
    ...
    function createStatementList(statementsOrHead) {
      for (var args = [],
        ...
        $traceurRuntime.setProperty(args, $__45 - 1, arguments[$traceurRuntime.toProperty($__45)]);
    ...
    function createCallCallStatement(func, thisExpression) {
      for (var args = [],
        ...
        $traceurRuntime.setProperty(args, $__46 - 2, arguments[$traceurRuntime.toProperty($__46)]);
    ...
    function createMemberExpression(operand, memberName, memberNames) {
      ...
      for (var i = 2; i < arguments.length; i++) {
        tree = createMemberExpression(tree, arguments[$traceurRuntime.toProperty(i)]);
      }
      return tree;
    }
    ...
    function createPropertyDescriptor(descr) {
      var propertyNameAndValues = Object.keys(descr).map(function(name) {
        var value = descr[$traceurRuntime.toProperty(name)];
    ...
    ($traceurRuntime.createClass)(ParseTreeTransformer, {
      ...
      transformList: function(list) {
        ...
        for (var index = 0; index < list.length; index++) {
          var element = list[$traceurRuntime.toProperty(index)];
  ...
  System.register("traceur@0.0.32/src/codegeneration/CoverFormalsTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(ToFormalParametersTransformer, {
      ...
      transformArrayLiteralExpression: function(tree) {
        ...
        for (var i = 0; i < okIndex; i++) {
          if (elements[$traceurRuntime.toProperty(i)] instanceof SpreadPatternElement)
            throw new CoverFormalsTransformerError(elements[$traceurRuntime.toProperty(i)].location, 'Unexpected token ...');
        }
    ...
    function toParenExpression(tree) {
      ...
      for (var i = 0; i < length; i++) {
        if (expressions[$traceurRuntime.toProperty(i)].isRestParameter())
          throw new CoverFormalsTransformerError(expressions[$traceurRuntime.toProperty(i)].location, 'Unexpected token ...');
      }
      var expression;
  ...
  System.register("traceur@0.0.32/src/util/ErrorReporter", [], function() {
    ...
    function format(location, text) {
      ...
      text = text.replace(/%./g, function(s) {
        switch (s) {
          case '%s':
            return args && args[$traceurRuntime.toProperty(i++)];
          case '%%':
            return '%';
        }
        return s;
    ...
    ($traceurRuntime.createClass)(Parser, {
      ...
      checkInitialisers_: function(variables) {
        if (parseOptions.blockBinding && variables.declarationType == CONST) {
          ...
          for (var i = 0; i < variables.declarations.length; i++) {
            if (!this.checkInitialiser_(type, variables.declarations[$traceurRuntime.toProperty(i)])) {
              break;
            }
          }
        }
      ...
      reportError_: function() {
        for (var args = [],
          ...
          $traceurRuntime.setProperty(args, $__64, arguments[$traceurRuntime.toProperty($__64)]);
        if (args.length == 1) {
  ...
  System.register("traceur@0.0.32/src/syntax/LineNumberTable", [], function() {
    ...
    function computeLineStartOffsets(source) {
      ...
      for (var index = 0; index < source.length; index++) {
        ...
        if (isLineTerminator(code)) {
          ...
          $traceurRuntime.setProperty(lineStartOffsets, k++, index + 1);
        }
      }
      $traceurRuntime.setProperty(lineStartOffsets, k++, MAX_INT_REPRESENTATION);
    ...
    ($traceurRuntime.createClass)(LineNumberTable, {
      ...
      getLine: function(offset) {
        ...
        if (offset < this.lastOffset_) {
          for (var i = this.lastLine_; i >= 0; i--) {
            if (this.lineStartOffsets_[$traceurRuntime.toProperty(i)] <= offset) {
              line = i;
              break;
            }
          }
      ...
      offsetOfLine: function(line) {
        this.ensureLineStartOffsets_();
        return this.lineStartOffsets_[$traceurRuntime.toProperty(line)];
      },
      getColumn: function(offset) {
        var line = this.getLine(offset);
        return offset - this.lineStartOffsets_[$traceurRuntime.toProperty(line)];
      },
  ...
  System.register("traceur@0.0.32/src/util/ArrayMap", [], function() {
    ...
    ($traceurRuntime.createClass)(ArrayMap, {
      ...
      get: function(key) {
        ...
        return this.values_[$traceurRuntime.toProperty(index)];
      },
      set: function(key, value) {
        var index = this.keys_.indexOf(key);
        if (index == -1) {
          ...
          $traceurRuntime.setProperty(this.values_, index, value);
        }
      },
      addAll: function(other) {
        var keys = other.keys();
        ...
        for (var i = 0; i < keys.length; i++) {
          this.set(keys[$traceurRuntime.toProperty(i)], values[$traceurRuntime.toProperty(i)]);
        }
      },
      remove: function(key) {
        var index = this.keys_.indexOf(key);
  ...
  System.register("traceur@0.0.32/src/codegeneration/PlaceholderParser", [], function() {
    ...
    function parseExpression(sourceLiterals) {
      for (var values = [],
        ...
        $traceurRuntime.setProperty(values, $__75 - 1, arguments[$traceurRuntime.toProperty($__75)]);
    ...
    function parseStatement(sourceLiterals) {
      for (var values = [],
        ...
        $traceurRuntime.setProperty(values, $__76 - 1, arguments[$traceurRuntime.toProperty($__76)]);
      return parse(sourceLiterals, values, (function() {
    ...
    function parseStatements(sourceLiterals) {
      for (var values = [],
        ...
        $traceurRuntime.setProperty(values, $__77 - 1, arguments[$traceurRuntime.toProperty($__77)]);
      return parse(sourceLiterals, values, (function() {
    ...
    function parsePropertyDefinition(sourceLiterals) {
      for (var values = [],
        ...
        $traceurRuntime.setProperty(values, $__78 - 1, arguments[$traceurRuntime.toProperty($__78)]);
      return parse(sourceLiterals, values, (function() {
    ...
    ($traceurRuntime.createClass)(PlaceholderParser, {
      ...
      parse_: function(sourceLiterals, doParse) {
        ...
        for (var i = 1; i < sourceLiterals.length; i++) {
          source += PREFIX + (i - 1) + sourceLiterals[$traceurRuntime.toProperty(i)];
        }
    ...
    ($traceurRuntime.createClass)(PlaceholderTransformer, {
      getValueAt: function(index) {
        return this.values[$traceurRuntime.toProperty(index)];
      },
  ...
  System.register("traceur@0.0.32/src/codegeneration/PrependStatements", [], function() {
    ...
    function prependStatements(statements) {
      for (var statementsToPrepend = [],
        ...
        $traceurRuntime.setProperty(statementsToPrepend, $__80 - 1, arguments[$traceurRuntime.toProperty($__80)]);
  ...
  System.register("traceur@0.0.32/src/codegeneration/TempVarTransformer", [], function() {
    ...
    function getVars(self) {
      var vars = self.tempVarStack_[$traceurRuntime.toProperty(self.tempVarStack_.length - 1)];
      if (!vars)
    ...
    ($traceurRuntime.createClass)(TempScope, {
      ...
      release: function(obj) {
        for (var i = this.identifiers.length - 1; i >= 0; i--) {
          obj.release_(this.identifiers[$traceurRuntime.toProperty(i)]);
        }
      }
    }, {});
    ...
    ($traceurRuntime.createClass)(TempVarTransformer, {
      transformStatements_: function(statements) {
        ...
        vars = vars.filter((function(tempVarStatement) {
          ...
          if ($traceurRuntime.toProperty(name) in seenNames) {
            if (seenNames[$traceurRuntime.toProperty(name)].initialiser || initialiser)
              throw new Error('Invalid use of TempVarTransformer');
            return false;
          }
          $traceurRuntime.setProperty(seenNames, name, tempVarStatement);
          return true;
        }));
      ...
      getTempIdentifier: function() {
        ...
        this.tempScopeStack_[$traceurRuntime.toProperty(this.tempScopeStack_.length - 1)].push(name);
        return name;
      },
      addTempVar: function() {
      ...
      addTempVarForThis: function() {
        var tempScope = this.tempScopeStack_[$traceurRuntime.toProperty(this.tempScopeStack_.length - 1)];
  ...
  System.register("traceur@0.0.32/src/codegeneration/module/ModuleSymbol", [], function() {
    ...
    ($traceurRuntime.createClass)(ExportsList, {
      addExport: function(name, tree) {
        assert(!this.exports_[$traceurRuntime.toProperty(name)]);
        $traceurRuntime.setProperty(this.exports_, name, tree);
      },
      getExport: function(name) {
        return this.exports_[$traceurRuntime.toProperty(name)];
      },
      getExports: function() {
    ...
    ($traceurRuntime.createClass)(ModuleSymbol, {
      addImport: function(name, tree) {
        assert(!this.imports_[$traceurRuntime.toProperty(name)]);
        $traceurRuntime.setProperty(this.imports_, name, tree);
      },
      getImport: function(name) {
        return this.imports_[$traceurRuntime.toProperty(name)];
      }
    }, {}, ExportsList);
    return {
  ...
  System.register("traceur@0.0.32/src/codegeneration/AnnotationsTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(AnnotationsTransformer, {
      ...
      get scope() {
        return this.stack_[$traceurRuntime.toProperty(this.stack_.length - 1)];
      },
    ...
    ($traceurRuntime.createClass)(VariableBinder, {
      bindVariablesInFunction_: function(tree) {
        ...
        for (var i = 0; i < parameters.length; i++) {
          this.bindParameter_(parameters[$traceurRuntime.toProperty(i)]);
        }
        this.visitAny(tree.functionBody);
      },
      visitBlock: function(tree) {
      ...
      visitVariableDeclarationList: function(tree) {
        if ((tree.declarationType == VAR && this.includeFunctionScope_) || (tree.declarationType != VAR && this.block_ == this.scope_)) {
          ...
          for (var i = 0; i < decls.length; i++) {
            this.visitAny(decls[$traceurRuntime.toProperty(i)].initialiser);
          }
        }
      },
      visitVariableDeclaration: function(tree) {
      ...
      bind_: function(identifier) {
        ...
        $traceurRuntime.setProperty(this.identifiers_, identifier.value, true);
      },
        ...
        switch (tree.type) {
          ...
          case ARRAY_PATTERN:
            ...
            for (var i = 0; i < elements.length; i++) {
              this.bindVariableDeclaration_(elements[$traceurRuntime.toProperty(i)]);
            }
            break;
          ...
          case OBJECT_PATTERN:
            ...
            for (var i = 0; i < fields.length; i++) {
              this.bindVariableDeclaration_(fields[$traceurRuntime.toProperty(i)]);
            }
            break;
  ...
  System.register("traceur@0.0.32/src/codegeneration/AlphaRenamer", [], function() {
    ...
    ($traceurRuntime.createClass)(AlphaRenamer, {
      transformBlock: function(tree) {
        if ($traceurRuntime.toProperty(this.oldName_) in variablesInBlock(tree)) {
          return tree;
        } else {
      ...
      getDoNotRecurse: function(tree) {
        return this.oldName_ === ARGUMENTS || this.oldName_ === THIS || $traceurRuntime.toProperty(this.oldName_) in variablesInFunction(tree);
      },
      transformCatch: function(tree) {
  ...
  System.register("traceur@0.0.32/src/codegeneration/ComprehensionTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(ComprehensionTransformer, {transformComprehension: function(tree, statement, isGenerator) {
        ...
        for (var i = tree.comprehensionList.length - 1; i >= 0; i--) {
          var item = tree.comprehensionList[$traceurRuntime.toProperty(i)];
          switch (item.type) {
  ...
  System.register("traceur@0.0.32/src/codegeneration/BlockBindingTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(Scope, {addBlockScopedVariable: function(value) {
        ...
        $traceurRuntime.setProperty(this.blockVariables, value, true);
      }}, {});
    ;
    ...
    ($traceurRuntime.createClass)(BlockBindingTransformer, {
        ...
        switch (comma.length) {
          ...
          default:
            for (var i = 0; i < comma.length; i++) {
              $traceurRuntime.setProperty(comma, i, createExpressionStatement(comma[$traceurRuntime.toProperty(i)]));
            }
      ...
      transformVariables_: function(tree) {
        ...
        for (var index = 0; index < variables.length; index++) {
          var variable = variables[$traceurRuntime.toProperty(index)];
          var variableName = this.getVariableName_(variable);
  ...
  System.register("traceur@0.0.32/src/semantics/util", [], function() {
    ...
    function hasUseStrict(list) {
      for (var i = 0; i < list.length; i++) {
        if (!list[$traceurRuntime.toProperty(i)].isDirectivePrologue())
          return false;
        if (list[$traceurRuntime.toProperty(i)].isUseStrictDirective())
          return true;
      }
      return false;
    }
  ...
  System.register("traceur@0.0.32/src/codegeneration/ExplodeExpressionTransformer", [], function() {
    ...
    function getResult(tree) {
      if (tree.type === COMMA_EXPRESSION)
        return tree.expressions[$traceurRuntime.toProperty(tree.expressions.length - 1)];
      return tree;
    }
    ...
    ($traceurRuntime.createClass)(ExplodeExpressionTransformer, {
      ...
      transformCommaExpression: function(tree) {
        ...
        for (var i = 0; i < expressions.length; i++) {
          builder.add(expressions[$traceurRuntime.toProperty(i)]);
        }
        return createCommaExpression($traceurRuntime.spread(builder.expressions, [getResult(expressions[$traceurRuntime.toProperty(expressions.length - 1)])]));
      },
      transformMemberExpression: function(tree) {
        var operand = this.transformAny(tree.operand);
      ...
      transformArrayLiteralExpression: function(tree) {
        ...
        var results = [];
        for (var i = 0; i < elements.length; i++) {
          builder.add(elements[$traceurRuntime.toProperty(i)]);
          results.push(getResult(elements[$traceurRuntime.toProperty(i)]));
        }
      ...
      transformObjectLiteralExpression: function(tree) {
        ...
        for (var i = 0; i < propertyNameAndValues.length; i++) {
          if (propertyNameAndValues[$traceurRuntime.toProperty(i)].type === PROPERTY_NAME_ASSIGNMENT) {
            builder.add(propertyNameAndValues[$traceurRuntime.toProperty(i)].value);
            results.push(new PropertyNameAssignment(propertyNameAndValues[$traceurRuntime.toProperty(i)].location, propertyNameAndValues[$traceurRuntime.toProperty(i)].name, getResult(propertyNameAndValues[$traceurRuntime.toProperty(i)].value)));
          } else {
            results.push(propertyNameAndValues[$traceurRuntime.toProperty(i)]);
          }
        }
      ...
      transformTemplateLiteralExpression: function(tree) {
        ...
        var results = [];
        for (var i = 0; i < elements.length; i++) {
          if (elements[$traceurRuntime.toProperty(i)].type === TEMPLATE_LITERAL_PORTION) {
            results.push(elements[$traceurRuntime.toProperty(i)]);
          } else {
            var expression = elements[$traceurRuntime.toProperty(i)].expression;
      ...
      transformCallMemberLookupExpression: function(tree) {
        ...
        args.args.forEach((function(arg, i) {
          ...
          if (tree.args.args[$traceurRuntime.toProperty(i)].type === SPREAD_EXPRESSION)
  ...
  System.register("traceur@0.0.32/src/codegeneration/CommonJsModuleTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(CommonJsModuleTransformer, {
      wrapModule: function(statements) {
        ...
        var last = statements[$traceurRuntime.toProperty(statements.length - 1)];
  ...
  System.register("traceur@0.0.32/src/codegeneration/ParameterTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(ParameterTransformer, {
      ...
      get parameterStatements() {
        return stack[$traceurRuntime.toProperty(stack.length - 1)];
      }
    }, {}, TempVarTransformer);
  ...
  System.register("traceur@0.0.32/src/codegeneration/DefaultParametersTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(DefaultParametersTransformer, {transformFormalParameterList: function(tree) {
        ...
        for (var i = 0; i < tree.parameters.length; i++) {
          var param = this.transformAny(tree.parameters[$traceurRuntime.toProperty(i)]);
          if (param !== tree.parameters[$traceurRuntime.toProperty(i)])
            changed = true;
          ...
          case ARRAY_PATTERN:
            {
              ...
              for (var i = 0; i < pattern.elements.length; i++) {
                var lvalue = pattern.elements[$traceurRuntime.toProperty(i)];
                if (lvalue === null) {
                  continue;
  ...
  System.register("traceur@0.0.32/src/codegeneration/generator/State", [], function() {
    ...
    State.replaceStateList = function(oldStates, oldState, newState) {
      ...
      for (var i = 0; i < oldStates.length; i++) {
        states.push(State.replaceStateId(oldStates[$traceurRuntime.toProperty(i)], oldState, newState));
      }
      return states;
    };
    ...
    State.replaceAllStates = function(exceptionBlocks, oldState, newState) {
      ...
      for (var i = 0; i < exceptionBlocks.length; i++) {
        result.push(exceptionBlocks[$traceurRuntime.toProperty(i)].replaceState(oldState, newState));
      }
      return result;
    };
    return {get State() {
    ...
    ($traceurRuntime.createClass)(TryState, {
      ...
      replaceNestedTrys: function(oldState, newState) {
        ...
        for (var i = 0; i < this.nestedTrys.length; i++) {
          states.push(this.nestedTrys[$traceurRuntime.toProperty(i)].replaceState(oldState, newState));
        }
        return states;
      }
    }, {});
  ...
  System.register("traceur@0.0.32/src/syntax/trees/StateMachine", [], function() {
    ...
    function addCatchOrFinallyStates(kind, enclosingMap, tryStates) {
      for (var i = 0; i < tryStates.length; i++) {
        var tryState = tryStates[$traceurRuntime.toProperty(i)];
        if (tryState.kind == kind) {
          for (var j = 0; j < tryState.tryStates.length; j++) {
            var id = tryState.tryStates[$traceurRuntime.toProperty(j)];
            $traceurRuntime.setProperty(enclosingMap, id, tryState);
          }
        }
    ...
    ($traceurRuntime.createClass)(StateMachine, {
      ...
      getAllStateIDs: function() {
        ...
        for (var i = 0; i < this.states.length; i++) {
          result.push(this.states[$traceurRuntime.toProperty(i)].id);
        }
        return result;
      },
      ...
      append: function(nextMachine) {
        ...
        for (var i = 0; i < nextMachine.states.length; i++) {
          var otherState = nextMachine.states[$traceurRuntime.toProperty(i)];
        ...
        for (var i = 0; i < nextMachine.exceptionBlocks.length; i++) {
          var tryState = nextMachine.exceptionBlocks[$traceurRuntime.toProperty(i)];
  ...
  System.register("traceur@0.0.32/src/codegeneration/HoistVariablesTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(HoistVariablesTransformer, {
      ...
      addVariable: function(name) {
        $traceurRuntime.setProperty(this.hoistedVariables_, name, true);
      },
      ...
      transformVariableDeclarationList: function(tree) {
        if (tree.declarationType == VAR) {
          ...
          for (var i = 0; i < declarations.length; i++) {
            var declaration = declarations[$traceurRuntime.toProperty(i)];
      ...
      addMachineVariable: function(name) {
        $traceurRuntime.setProperty(this.machineVariables_, name, true);
      },
      transformFunctionDeclaration: function(tree) {
        return tree;
      },
  ...
  System.register("traceur@0.0.32/src/codegeneration/generator/BreakState", [], function() {
    ...
    ($traceurRuntime.createClass)(BreakState, {
      ...
      transformBreak: function(labelSet) {
        ...
        if ($traceurRuntime.toProperty(this.label) in labelSet) {
          return new FallThroughState(this.id, labelSet[$traceurRuntime.toProperty(this.label)].fallThroughState, []);
        }
        return this;
      },
      transformBreakOrContinue: function(labelSet) {
  ...
  System.register("traceur@0.0.32/src/codegeneration/generator/ContinueState", [], function() {
    ...
    ($traceurRuntime.createClass)(ContinueState, {
      ...
      transformBreakOrContinue: function(labelSet) {
        ...
        if ($traceurRuntime.toProperty(this.label) in labelSet) {
          return new FallThroughState(this.id, labelSet[$traceurRuntime.toProperty(this.label)].continueState, []);
        }
        return this;
      }
    }, {}, State);
    ...
    ($traceurRuntime.createClass)(SwitchState, {
      ...
      transform: function(enclosingFinally, machineEndState, reporter) {
        ...
        for (var i = 0; i < this.clauses.length; i++) {
          var clause = this.clauses[$traceurRuntime.toProperty(i)];
    ...
    ($traceurRuntime.createClass)(CPSTransformer, {
      ...
      transformBlock: function(tree) {
        ...
        if (label) {
          ...
          for (var i = 0; i < machine.states.length; i++) {
            var state = machine.states[$traceurRuntime.toProperty(i)];
      ...
      transformStatementList_: function(trees) {
        ...
        for (var i = 0; i < trees.length; i++) {
          if (trees[$traceurRuntime.toProperty(i)].type === STATE_MACHINE) {
            groups.push(trees[$traceurRuntime.toProperty(i)]);
          } else if (needsStateMachine(trees[$traceurRuntime.toProperty(i)])) {
            newMachine = this.ensureTransformed_(trees[$traceurRuntime.toProperty(i)]);
            groups.push(newMachine);
          } else {
            var last = groups[$traceurRuntime.toProperty(groups.length - 1)];
            if (!(last instanceof Array))
              groups.push(last = []);
            last.push(trees[$traceurRuntime.toProperty(i)]);
          }
        }
        ...
        for (var i = 0; i < groups.length; i++) {
          if (groups[$traceurRuntime.toProperty(i)] instanceof Array) {
            newMachine = this.statementsToStateMachine_(groups[$traceurRuntime.toProperty(i)]);
          } else {
            newMachine = groups[$traceurRuntime.toProperty(i)];
          }
          if (i === 0)
            machine = newMachine;
          else
      ...
      needsStateMachine_: function(statements) {
        if (statements instanceof Array) {
          for (var i = 0; i < statements.length; i++) {
            if (needsStateMachine(statements[$traceurRuntime.toProperty(i)]))
              return true;
          }
          return false;
        }
      ...
      addLoopBodyStates_: function(loopBodyMachine, continueState, breakState, labels, states) {
        for (var i = 0; i < loopBodyMachine.states.length; i++) {
          var state = loopBodyMachine.states[$traceurRuntime.toProperty(i)];
      ...
      removeEmptyStates: function(oldStates) {
        var emptyStates = [],
            newStates = [];
        for (var i = 0; i < oldStates.length; i++) {
          if (oldStates[$traceurRuntime.toProperty(i)] instanceof FallThroughState && oldStates[$traceurRuntime.toProperty(i)].statements.length === 0) {
            emptyStates.push(oldStates[$traceurRuntime.toProperty(i)]);
          } else {
            newStates.push(oldStates[$traceurRuntime.toProperty(i)]);
          }
        }
        for (i = 0; i < newStates.length; i++) {
          $traceurRuntime.setProperty(newStates, i, emptyStates.reduce((function(state, $__233) {
            var id = $__233.id,
                fallThroughState = $__233.fallThroughState;
            return state.replaceState(id, fallThroughState);
          }), newStates[$traceurRuntime.toProperty(i)]));
        }
        return newStates;
      },
      replaceAndAddStates_: function(oldStates, oldState, newState, newStates) {
        for (var i = 0; i < oldStates.length; i++) {
          newStates.push(oldStates[$traceurRuntime.toProperty(i)].replaceState(oldState, newState));
        }
      },
      transformLabelledStatement: function(tree) {
        var startState = this.allocateState();
      ...
      addLabel_: function(label) {
        ...
        for (var k in this.labelSet_) {
          $traceurRuntime.setProperty(labelSet, k, this.labelSet_[$traceurRuntime.toProperty(k)]);
        }
        $traceurRuntime.setProperty(labelSet, label.name, label);
      ...
      transformSwitchStatement: function(tree) {
        ...
        for (var index = caseClauses.length - 1; index >= 0; index--) {
          var clause = caseClauses[$traceurRuntime.toProperty(index)];
      ...
      transformMachineStates: function(machine, machineEndState, rethrowState, enclosingFinallyState) {
        ...
        for (var i = 0; i < machine.states.length; i++) {
          var state = machine.states[$traceurRuntime.toProperty(i)];
          var stateCase = state.transformMachineState(enclosingFinallyState[$traceurRuntime.toProperty(state.id)], machineEndState, this.reporter);
          if (stateCase != null) {
          ...
          if (tryState.kind == TryState.Kind.FINALLY) {
            ...
            if (enclosingFinallyState != null) {
              ...
              for (var j = 0; j < enclosingFinallyState.tryStates.length; j++) {
                var destination = enclosingFinallyState.tryStates[$traceurRuntime.toProperty(j)];
                index++;
                var statements;
      ...
      ensureTransformedList_: function(statements) {
        ...
        for (var i = 0; i < statements.length; i++) {
          var statement = statements[$traceurRuntime.toProperty(i)];
    ...
    ($traceurRuntime.createClass)(ObjectLiteralTransformer, {
      findSeenAccessor_: function(name) {
        if (name.type === COMPUTED_PROPERTY_NAME)
          return null;
        var s = propName(name);
        return this.seenAccessors[$traceurRuntime.toProperty(s)];
      },
      removeSeenAccessor_: function(name) {
        if (name.type === COMPUTED_PROPERTY_NAME)
          return;
        var s = propName(name);
        delete this.seenAccessors[$traceurRuntime.toProperty(s)];
      },
      addSeenAccessor_: function(name, descr) {
        if (name.type === COMPUTED_PROPERTY_NAME)
          return;
        var s = propName(name);
        $traceurRuntime.setProperty(this.seenAccessors, s, descr);
      },
  ...
  System.register("traceur@0.0.32/src/codegeneration/RestParameterTransformer", [], function() {
    ...
    function hasRestParameter(formalParameterList) {
      ...
      return parameters.length > 0 && parameters[$traceurRuntime.toProperty(parameters.length - 1)].isRestParameter();
    }
    function getRestParameterLiteralToken(formalParameterList) {
      var parameters = formalParameterList.parameters;
      return parameters[$traceurRuntime.toProperty(parameters.length - 1)].parameter.identifier.identifierToken;
    }
  ...
  System.register("traceur@0.0.32/src/codegeneration/SpreadTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(SpreadTransformer, {
      createArrayFromElements_: function(elements, needsNewArray) {
        ...
        var lastArray;
        for (var i = 0; i < length; i++) {
          if (elements[$traceurRuntime.toProperty(i)] && elements[$traceurRuntime.toProperty(i)].type === SPREAD_EXPRESSION) {
            if (lastArray) {
              args.push(createArrayLiteralExpression(lastArray));
              lastArray = null;
            }
            args.push(this.transformAny(elements[$traceurRuntime.toProperty(i)].expression));
          } else {
            if (!lastArray)
              lastArray = [];
            lastArray.push(this.transformAny(elements[$traceurRuntime.toProperty(i)]));
          }
        }
        if (lastArray)
          args.push(createArrayLiteralExpression(lastArray));
  ...
  System.register("traceur@0.0.32/src/codegeneration/TemplateLiteralTransformer", [], function() {
    ...
    function maybeAddEmptyStringAtEnd(elements, items) {
      var length = elements.length;
      if (!length || elements[$traceurRuntime.toProperty(length - 1)].type !== TEMPLATE_LITERAL_PORTION)
        items.push(createStringLiteral(''));
    }
    function createRawStringArray(elements) {
      var items = [];
      for (var i = 0; i < elements.length; i += 2) {
        var str = replaceRaw(JSON.stringify(elements[$traceurRuntime.toProperty(i)].value.value));
        var loc = elements[$traceurRuntime.toProperty(i)].location;
    ...
    function createCookedStringArray(elements) {
      var items = [];
      for (var i = 0; i < elements.length; i += 2) {
        items.push(createCookedStringLiteralExpression(elements[$traceurRuntime.toProperty(i)]));
      }
      maybeAddEmptyStringAtEnd(elements, items);
      return createArrayLiteralExpression(items);
    }
    ...
    function cookString(s) {
      ...
      var i = 0,
          ...
          c2;
      while (i < s.length) {
        c = s[$traceurRuntime.toProperty(i++)];
        switch (c) {
          case '\\':
            c2 = s[$traceurRuntime.toProperty(i++)];
            switch (c2) {
              case '\n':
              case '\u2028':
              case '\u2029':
                break;
              case '\r':
                if (s[$traceurRuntime.toProperty(i + 1)] === '\n') {
                  i++;
                }
                break;
              default:
                $traceurRuntime.setProperty(sb, k++, c);
                $traceurRuntime.setProperty(sb, k++, c2);
            }
            break;
          case '"':
            $traceurRuntime.setProperty(sb, k++, '\\"');
            break;
          case '\n':
            $traceurRuntime.setProperty(sb, k++, '\\n');
            break;
          case '\r':
            $traceurRuntime.setProperty(sb, k++, '\\r');
            break;
          case '\t':
            $traceurRuntime.setProperty(sb, k++, '\\t');
            break;
          case '\f':
            $traceurRuntime.setProperty(sb, k++, '\\f');
            break;
          case '\b':
            $traceurRuntime.setProperty(sb, k++, '\\b');
            break;
          case '\u2028':
            $traceurRuntime.setProperty(sb, k++, '\\u2028');
            break;
    ...
    ($traceurRuntime.createClass)(TemplateLiteralTransformer, {
      ...
      transformTemplateLiteralExpression: function(tree) {
        ...
        for (var i = 1; i < elements.length; i += 2) {
          args.push(this.transformAny(elements[$traceurRuntime.toProperty(i)]));
        }
      ...
      createDefaultTemplateLiteral: function(tree) {
        ...
        for (var i = 1; i < length; i++) {
          ...
          var transformedTree = this.transformAny(tree.elements[$traceurRuntime.toProperty(i)]);
  ...
  System.register("traceur@0.0.32/src/codegeneration/TypeAssertionTransformer", [], function() {
    ...
    ($traceurRuntime.createClass)(TypeAssertionTransformer, {
      ...
      get returnType_() {
        return this.returnTypeStack_.length > 0 ? this.returnTypeStack_[$traceurRuntime.toProperty(this.returnTypeStack_.length - 1)] : null;
      },
      get paramTypes_() {
        return this.parametersStack_[$traceurRuntime.toProperty(this.parametersStack_.length - 1)];
      }
    }, {}, ParameterTransformer);
    ...
    ($traceurRuntime.createClass)(ExportListBuilder, {buildExportList: function(deps, loader) {
        ...
        function doVisit(ctor) {
          for (var i = 0; i < deps.length; i++) {
            var visitor = new ctor(reporter, loader, deps[$traceurRuntime.toProperty(i)].moduleSymbol);
            visitor.visitAny(deps[$traceurRuntime.toProperty(i)].tree);
          }
        }
  ...
  System.register("traceur@0.0.32/src/codegeneration/module/ModuleSpecifierVisitor", [], function() {
    ...
    ($traceurRuntime.createClass)(ModuleSpecifierVisitor, {
      ...
      visitModuleSpecifier: function(tree) {
        $traceurRuntime.setProperty(this.moduleSpecifiers_, tree.token.processedValue, true);
      },
      visitVariableDeclaration: function(tree) {
      ...
      addTypeAssertionDependency_: function(typeAnnotation) {
        if (typeAnnotation !== null && options.typeAssertionModule !== null)
          $traceurRuntime.setProperty(this.moduleSpecifiers_, options.typeAssertionModule, true);
      }
    }, {}, ParseTreeVisitor);
  ...
  System.register("traceur@0.0.32/src/runtime/system-map", [], function() {
    ...
    function prefixMatchLength(name, prefix) {
      ...
      for (var i = 0; i < prefixParts.length; i++) {
        if (nameParts[$traceurRuntime.toProperty(i)] != prefixParts[$traceurRuntime.toProperty(i)])
          return 0;
      }
    ...
    function applyMap(map, name, parentName) {
      ...
      if (parentName) {
        ...
        Object.getOwnPropertyNames(map).some(function(p) {
          var curMap = map[$traceurRuntime.toProperty(p)];
          ...
          if (curMatch) {
            ...
            mappedName = map[$traceurRuntime.toProperty(curParent)][$traceurRuntime.toProperty(curMatch)] + (subPath ? '/' + subPath : '');
            return mappedName;
          }
        });
      }
      ...
      if (!curMatch)
        return name;
      var subPath = name.split('/').splice(curMatchLength).join('/');
      return map[$traceurRuntime.toProperty(curMatch)] + (subPath ? '/' + subPath : '');
    }
    ...
    ($traceurRuntime.createClass)(LoaderHooks, {
      ...
      analyzeDependencies: function(dependencies, loader) {
        ...
        for (var i = 0; i < dependencies.length; i++) {
          var codeUnit = dependencies[$traceurRuntime.toProperty(i)];
      ...
      bundledModule: function(name) {
        return this.moduleStore_.bundleStore[$traceurRuntime.toProperty(name)];
      }
    }, {});
  ...
  System.register("traceur@0.0.32/src/runtime/InterceptOutputLoaderHooks", [], function() {
    ...
    var InterceptOutputLoaderHooks = function InterceptOutputLoaderHooks() {
      for (var args = [],
        ...
        $traceurRuntime.setProperty(args, $__322, arguments[$traceurRuntime.toProperty($__322)]);
  ...
  System.register("traceur@0.0.32/src/runtime/InternalLoader", [], function() {
    ...
    ($traceurRuntime.createClass)(InternalLoader, {
      ...
      getKey: function(url, type) {
        ...
        if ($traceurRuntime.toProperty(combined) in this.urlToKey) {
          return this.urlToKey[$traceurRuntime.toProperty(combined)];
        }
        return $traceurRuntime.setProperty(this.urlToKey, combined, {});
      },
    ...
    ($traceurRuntime.createClass)(WebPageTranscoder, {
      ...
      addFilesFromScriptElements: function(scriptElements, onScriptsReady) {
        for (var i = 0,
          ...
          var scriptElement = scriptElements[$traceurRuntime.toProperty(i)];
      ...
      semverMap: function(normalizedName) {
        ...
        if (at !== -1) {
          ...
          if (m) {
            ...
            $traceurRuntime.setProperty(map, packageName, version);
            $traceurRuntime.setProperty(map, packageName + '@' + major, version);
            $traceurRuntime.setProperty(map, packageName + '@' + major + '.' + minor, version);
          }
        }
        return map;
      },
  ...
  return {};
}.bind(typeof global !== 'undefined' ? global : this));
